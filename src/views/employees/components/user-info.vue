    ...
    <el-form label-width="220px">
      <div class="block">
        ...
        <el-form-item label="员工照片">
          <!-- 放置上传图片 -->
          <image-upload ref="peoplePhotoRef" />
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    async saveUser() {
      // 获取头像地址
      const fileList = this.$refs.staffPhotoRef.fileList // 数组
      // 应该做一个判断，判断当前的图片没有上传完成
      if (fileList.some(item => !item.upload)) {
        // 说明有图片没有上传完成
        this.$message.warning('此时还有图片没完成')
        return
      }
      // 由于接口原因，当无图片上传时将地址设置为空格
      this.userInfo = { ...this.userInfo, staffPhoto: fileList[0] ? fileList[0].url : ' ' }
      await saveUserDetailById(this.userInfo)
      this.$message.success('保存成功')
    },
    async savePersonal() {
      // 获取头像地址
      const fileList = this.$refs.peoplePhotoRef.fileList // 数组
      // 应该做一个判断，判断当前的图片没有上传完成
      if (fileList.some(item => !item.upload)) {
        // 说明有图片没有上传完成
        this.$message.warning('此时还有图片没完成')
      ...
      // 由于接口原因，当无图片上传时将地址设置为空格
      this.formData = { ...this.formData, staffPhoto: fileList[0] ? fileList[0].url : ' ' }
